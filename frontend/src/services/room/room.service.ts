...
export const roomService = {
  async createRoom(data: CreateRoomRequest): Promise<Room> {
    logger.info("RoomService: Creating room", {
      name: data.name,
      is_private: data.is_private,
    });
    const response = await apiClient.post<Room | { room: Room }>(
      "/rooms",
      data,
    );
  ...
  async getRoom(roomId: string): Promise<Room> {
    ...
    const response = await apiClient.get<Room | { room: Room }>(
      `/rooms/${roomId}`,
    );
    if (response && "room" in response && response.room) {
      return response.room;
  ...
  async listRooms(): Promise<Room[]> {
    try {
      ...
      const response = await apiClient.get<Room[] | { rooms: Room[] }>(
        "/rooms",
      );
  ...
  async updateMemberRole(
    ...
    await apiClient.put<void>(`/rooms/${roomId}/members/${userId}/role`, {
      role,
    });
  },
  ...
  async inviteUser(roomId: string, username: string): Promise<RoomInvitation> {
    ...
    return apiClient.post<RoomInvitation>(`/rooms/${roomId}/invite`, {
      username,
    });
  },
  ...
  async acceptInvitation(invId: string): Promise<{ room: Room; member: any }> {
    ...
    return apiClient.post<{ room: Room; member: any }>(
