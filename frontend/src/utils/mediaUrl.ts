...
export function normalizeMediaUrl(url: string | undefined | null): string {
  ...
  if (
    url.includes("localhost:9000/inox-media") ||
    url.includes("127.0.0.1:9000/inox-media")
  ) {
    const cleanPath = url.replace(
      /^http:\/\/(localhost|127\.0\.0\.1):9000\/inox-media\/?/,
      "",
    );
  ...
  if (
    streamBase.includes("trycloudflare.com") ||
    streamBase.startsWith("https://")
  ) {
    ...
    if (url.includes("localhost:8080/inox-media")) {
      const cleanPath = url.replace(
        /^http:\/\/localhost:8080\/inox-media\/?/,
        "",
      );
      return `${streamBase.replace(/\/$/, "")}/${cleanPath}`;
    }
