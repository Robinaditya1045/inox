...
import { LiveChannelCenter } from "@/components/live/LiveChannelCenter";
...
export function App() {
  ...
  return (
    ...
    >
      ...
      {activeTab === "live" && <LiveChannelCenter />}

      {activeTab === "rooms" && (
        <RoomCommandCenter liveRooms={telemetry.current?.rooms} />
      )}
      ...
      {activeTab === "debug" && (
        <Card className="border-zinc-800">
          <CardHeader>
            ...
            <CardDescription>
              Real-time terminal log viewer filtered by Room ID, User ID, and
              Trace-ID across HTTP, WebSocket, and WebRTC signaling layers.
            </CardDescription>
          </CardHeader>
          <CardContent className="py-12 text-center text-zinc-500 font-mono text-xs">
            <p className="text-emerald-400/80">
              root@inox-admin:~# tail -f /var/log/inox/telemetry.log
            </p>
